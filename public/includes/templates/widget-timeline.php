<ul class="tweet-list">
<?php
foreach($tweets as $tweet):
	$tweet_id = $tweet->id; ?>
    <li class="of-tweet">
    	<div id="tweet-<?php echo $tweet_id; ?>" class="tweet-content">
        	<?php 
        	$text = preg_replace('#@(\w+)#', '<a target="_blank" href="https://twitter.com/$1">$0</a>', $tweet->text);
			$text = preg_replace('/#(\w+)/', '<a target="_blank" href="https://twitter.com/search?q=%23$1&src=hash">$0</a>', $text);
			echo $text; ?>
        </div>
		<div class="tweet-meta">
        	<span class="time-meta"><a href="https://twitter.com/<?php echo $tweet->user->screen_name; ?>/statuses/<?php echo $tweet_id; ?>" target="_blank"><?php echo OF_Social_Connect::time_ago(strtotime($tweet->created_at)); ?></a></span>       
        	<?php /*<span class="from-meta">from <?php echo $tweet->source; ?></span>*/ ?>
        </div> 
        <div class="tweet-intent">
            <a href="https://twitter.com/intent/tweet?in_reply_to=<?php echo $tweet_id; ?>" class="in-reply-to" title="Reply" target="_blank"><span class="sr-only">Reply</span></a>
            <a href="https://twitter.com/intent/retweet?tweet_id=<?php echo $tweet_id; ?>" class="retweet" title="Retweet" target="_blank"><span class="sr-only">Retweet</span></a>
            <a href="https://twitter.com/intent/favorite?tweet_id=<?php echo $tweet_id; ?>" class="favorite" title="Favorite" target="_blank"><span class="sr-only">Favorite</span></a>
        </div>
    </li>
<?php endforeach;	
?>
</ul>